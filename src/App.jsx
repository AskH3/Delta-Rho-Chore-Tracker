...
// Completions may have the old single `photo_path` or the new `photo_paths`
// array. Always resolve to an array of URLs so the UI has one code path.
function photoUrls(c) {
  const paths =
    c.photo_paths && c.photo_paths.length
      ? c.photo_paths
      : c.photo_path
      ? [c.photo_path]
      : [];
  return paths.map(photoUrl);
}
...
function CompletionModal({
  ...
  // items: { key, blob?, url, isExisting } — existing items keep their
  // storage path in `path`; new items get uploaded on submit.
  const [items, setItems] = useState(() =>
    existing
      ? (existing.photo_paths && existing.photo_paths.length
  ...
  const pick = async (e) => {
    ...
    if (room <= 0) return setErr(`You can attach up to ${MAX_PHOTOS} photos.`);
    ...
    try {
      ...
      setErr("Couldn't process one of those images. Try again.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const submit = async () => {
    if (!items.length)
      return setErr("At least one photo of the finished chore is required.");
    setBusy(true);
    setErr("");
    try {
      const paths = [];
      ...
      if (existing) {
        const { error } = await supabase
          .from("completions")
          .update({ photo_paths: paths, photo_path: paths[0] })
          .eq("id", existing.id);
        if (error) throw error;
      } else {
    ...
    <Modal onClose={onClose}>
      ...
      {items.length < MAX_PHOTOS && (
        <Btn
          ...
          disabled={busy}
          style={{ width: "100%", padding: items.length ? "9px 0" : "14px 0" }}
        >
          {busy
            ? "Processing…"
            : items.length
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
        <Btn kind="gold" onClick={submit} disabled={busy || !items.length}>
...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
function PhotoStack({ urls, alt, accent }) {
      ...
      {urls.length > 1 && (
        <>
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              setI((idx - 1 + urls.length) % urls.length);
            }}
            style={{
              ...btnBase,
              position: "absolute",
            ...
            {urls.map((_, di) => (
              <div
                ...
                style={{
                  ...
                  height: 5,
                  borderRadius: "50%",
                  background: di === idx ? C.white : "rgba(255,255,255,0.45)",
                }}
              />
            ))}
          </div>
...
function Feed({ profile, refreshFlag }) {
  ...
  const remove = async (comp) => {
    ...
    const paths =
      comp.photo_paths && comp.photo_paths.length
        ? comp.photo_paths
        : comp.photo_path
        ? [comp.photo_path]
        : [];
          ...
          return (
            ...
            >
              <PhotoStack
                ...
                accent={c.section === "newboy" ? C.gold : C.cardinal}
              />
